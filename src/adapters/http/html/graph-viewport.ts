...
import { ACTUAL_SIZE_ZOOM, describeGraphZoom, FIT_ZOOM, stepGraphZoom, type GraphZoom } from './graph-zoom.ts';
...
export function renderGraphViewport(graph: FlowGraph, projectCode: string, state: ProjectViewState, selectedVariantId: string | undefined): string {
  ...
  const isAuto = state.zoom.kind === 'auto';
...
${zoomLink('等倍', '等倍で表示', 'reset', href(ACTUAL_SIZE_ZOOM), isActual)}
...
<div class="graph-scroll${isFit ? ' fit' : ''}${isAuto ? ' auto' : ''}" tabindex="0" data-graph-scroll aria-label="グラフ表示領域 (矢印キーで移動、+ と - で拡大縮小、0 で全体表示)">
