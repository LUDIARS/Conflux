# Web UI (表示・入力 adapter)

対応仕様: Pf CF-WEB-001 (`01M2SMDQD5060SPPKSK6QFTSCG`)、CF-DESIGN-001 の Web/Tela 方針。価値: CF-UX-1〜4 の入口。
所属ドメイン: platform-foundation (`src/adapters/http/html/**`、`tests/adapters/**`)。
**以下の受入基準は設計案。テストとヘッドレスブラウザでのレイアウト実測は済み、実機・支援技術・UX 評価は未実施 (未確認)。**

## 位置付け

Conflux は WebUI と Tela のオーバーレイ UI の両方を持つ。どちらも同じ use case と読み取りモデル
(`loadProjectOverview` ほか 7 ドメインの application) を使う **表示・入力 adapter** であり、
業務ルール・履歴・権限判定を UI ごとに複製しない。業務状態の所有者は既存 7 ドメインのまま。

| UI | 範囲 | 状態 |
|---|---|---|
| Web (`src/adapters/http/html/**`) | PC とスマホのブラウザ。SSR HTML + 最小の補助スクリプト | 今回実装 |
| Tela overlay | Tela ホストへの接続とオーバーレイ表示 | **後続 (未実装)**。Web と同じ use case / read model を呼ぶ別 adapter として追加する |

権限 (デプロイの管理職以上など) はサーバー側の use case が Cc の応答で判定する。UI は判定しない。
実接続できない spawn / ビルド / デプロイ / harness 選択は UI でも成功表示しない (I-3)。

## 画面構成

プロジェクト選択 (`GET /`) → 流れのグラフ (`GET /projects/:code`) → 選択した潮流/亜流の詳細。

- **PC (幅 960px 以上)**: グラフ面と詳細面を横に並べ、見比べられる。グラフ面は画面上部に留める。
- **スマホ・タブレット (960px 未満)**: 1 面ずつ表示し、「グラフ / 詳細」の切替で行き来する。
  切替・タブ・拡大率の変更は同じ選択 id (`variant`) を保つ。
- 詳細はタブで 6 区分に分ける: コンセプト / ルール差分 (比較・改修) / 対話 (コメント・評価) /
  成果物 (ビルド・デプロイ) / 作業依頼 (Cc spawn) / 合流判断。
- グラフを補う **流れ一覧** (潮流ごとの亜流一覧) も同じ `variant` へ遷移する。
- 技術設定 (Cf Flow の Cc 報告・本流命名・再照会) は折りたたみに置き、初期画面の主役にしない。

## 画面状態 (URL)

`/projects/:code?variant=<id>&view=graph|detail&tab=<tab>&zoom=fit|0.5|0.75|1|1.5|2&compare=<id>`

`zoom` を省いたときが既定の「画面に合わせる」(auto) で、URL には現れない。

- 状態は URL だけが持つ (サーバー状態を増やさない)。リロード・共有・戻る操作で同じ画面になる。
- 不正な `view` / `tab` / `zoom` は既定値 (`graph` / `concept` / auto) に戻す。
- POST 後の再表示 (PRG) は同じ `variant` と、操作した区分のタブ・`view=detail` へ戻す。

## グラフ操作

- **グラフの向き** (2026-09-19 neco 指示): PC (幅 960px 以上) は流れの進みを右方向、潮流の分岐を縦に並べる
  (正式ルールは左端)。スマホ・タブレット (960px 未満) は流れの進みを上方向、潮流の分岐を横に並べる
  (正式ルールは下端)。配置は `graph-layout.ts` が潮流 (lane) と進み (step) で決め、向きごとに座標へ写す。
  両方の向きの SVG を出し、スタイルで片方だけ表示する (幅が変わっても再読込不要。非表示側はフォーカス・
  読み上げの対象外)。選択ノードの中央寄せは表示中の側で行う。
- **既定の表示倍率** (2026-09-20): `zoom` 省略時は「画面に合わせる」(auto)。PC の右向きグラフだけを
  ペイン幅に収め、スマホの上向きグラフは描画寸法のままスクロールさせる。向きごとに破綻の仕方が逆で、
  右向きは短いペインの右端から外れて到達時に見えず、上向きはスマホ幅へ収めると文字が読めなくなるため。
  適用はスタイルシートで行い、両方の向きの SVG を出す構成のまま script も再読込も要らない。
  実測 (2026-09-20 ヘッドレス Edge、3 潮流 10 ノード): 等倍のままだと 1280px でグラフ 920px に対し
  ペイン 588px、ノード 6 件が右端の外。auto では 1280px で切れ 0 件、スマホ幅は従来どおり。
- 全体表示 (`zoom=fit`)・拡大・縮小・等倍をリンクで提供する (JS 無しでも動く)。`zoom=fit` は
  読み手が自分で選んだときだけ効き、幅に関わらず縮小する (スマホでは文字が小さくなる代わりに全体が入る)。
- グラフ領域は内部スクロールで移動でき、文書全体は横にはみ出さない。移動ボタン (上下左右) は
  補助スクリプトで有効化し、スクロール領域はキーボード (矢印、`+` / `-` / `0`) でも操作できる。
- touch だけ・hover だけに依存する操作を必須にしない。ノード名はリンクの accessible name と
  `<title>` に全文を持ち、長い名前は図上で省略する。

## 受入基準案

| ID | 基準 | 確認方法 |
|---|---|---|
| W-1 | 幅 320 / 390 / 768 / 1280px で文書全体の横はみ出しがない (グラフ領域の内部移動は可) | 2026-09-18 ヘッドレス Edge 実測で 12 画面 × 4 幅すべて横はみ出し 0 (下記) |
| W-2 | 主操作のタップ領域 44px 以上、入力欄の文字 16px 以上 | 同実測で入力欄 16px 未満 0。44px 未満は skip-link (非表示時) と「全体表示」時の図上ノードのみ (下記)。単体テスト通過 |
| W-3 | 表示切替・タブ・拡大率・流れ一覧の操作で選択 id を失わない | 単体テスト通過 (2026-09-18、108/108) |
| W-4 | 全入力に label、フォーカス表示、キーボード操作 | 静的テスト通過。フォーカス順の目視・スクリーンリーダーでの確認は未実施 |
| W-5 | 長いタイトル・ブランチ名・コメント、多数ノード、空状態、存在しない選択、送信失敗/オフラインを扱う | 単体テスト通過、長文・31 ノード・空状態・存在しない選択はレイアウト実測済み。実機は未実施 |

### 2026-09-18 レイアウト実測の範囲

- 方法: SSR 出力 (サーバー未起動) をヘッドレス Edge の iframe に幅 320/390/768/1280px で読み込み、
  `documentElement.scrollWidth`、スクロール領域外で画面外に出る要素、操作要素の寸法、入力欄の文字サイズを計測した。
  対象は一覧・グラフ (等倍/全体表示)・詳細 6 タブ・空状態・存在しない選択の 12 画面。長い日本語名、40 文字の
  ブランチ識別名、1 潮流 30 亜流、長いルール本文を含む。
- 実測で見つけた成果物リンク (本文内リンクで 44px 未満) は `button-link` にして解消した。
- 「全体表示」では図上ノードが縮小されて 44px 未満になる。全体把握用の表示であり、同じ選択へは流れ一覧 (44px) から
  遷移できるため仕様どおりとする。
- **未確認**: 実機タッチでのグラフ移動・拡大縮小、スクリーンリーダー、フォーカス順の目視、実ブラウザでの
  補助スクリプト動作 (移動ボタン・キー操作)、通信失敗時の表示。UX 達成 (CF-UX-1〜4) は人間の評価前であり未確認。

## 実装

| 役割 | ファイル |
|---|---|
| 画面状態 (URL の解析・生成) | `src/adapters/http/html/view-state.ts` |
| 拡大率 | `src/adapters/http/html/graph-zoom.ts` |
| SVG 描画・ラベル省略 | `graph-svg.ts`, `svg-label.ts` |
| グラフ操作領域 | `graph-viewport.ts` |
| 流れ一覧 | `flow-list.ts` |
| グラフ面 / 詳細面 / タブ | `graph-pane.ts`, `detail-pane.ts`, `detail-tabs.ts`, `tab-panels.ts` |
| 見出し・設定 | `project-header.ts` |
| フォーム部品 | `form-controls.ts`, `variant-forms.ts`, `flow-forms.ts` |
| 骨格・CSS・補助スクリプト | `layout.ts`, `styles.ts`, `client-script.ts` |
| プロジェクト一覧 | `project-index.ts` |

テスト: `tests/adapters/web-ui.test.ts`, `tests/adapters/http-app.test.ts`。
