...
本流の物理命名は 2026-09-20 に `evolution/x/y/main` (`mainline: suffix-main`、作業接頭辞 `feature`) と
確定した。git は `evolution/x/y` とその配下を同時に持てず、亜流自身を本流にすると配下の名前空間を
恒久的に塞ぐため。値はプロジェクト設定のままで、未指定のときにこの既定が入る。
